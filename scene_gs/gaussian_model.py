...
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'submodules', 'simple-knn'))
...
from scene_gs.neural_renderer import GridRenderer
from scene_gs.motion_net import PersonalizedMotionNetwork
...
class GaussianModel:
    ...
    def get_scaling(self):
        if self._scaling is None or len(self._scaling) == 0:
            return torch.ones(1, 3, device='cuda') * 0.01
        return self.scaling_activation(self._scaling)
    
    @property
    def get_rotation(self):
        if self._rotation is None or len(self._rotation) == 0:
            return torch.tensor([[1, 0, 0, 0]], device='cuda', dtype=torch.float32)
        return self.rotation_activation(self._rotation).float()
    ...
    def get_features(self):
        if self._features_dc is None or len(self._features_dc) == 0:
            return torch.zeros(1, 1, 3, device='cuda')
    ...
    def get_identity(self):
        if self._identity is None or len(self._identity) == 0:
            return torch.zeros(1, 1, device='cuda', dtype=torch.long)
        return self._identity
    
    @property
    def get_opacity(self):
        if self._opacity is None or len(self._opacity) == 0:
            return torch.ones(1, 1, device='cuda') * 0.5
