...
if __name__ == '__main__':
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            # convert train log + eval csv into best and balanced checkpoint selection
            convert_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scripts", "convertlog2eval.py")
            if os.path.exists(convert_script) and hasattr(trainer, "log_path") and os.path.exists(trainer.log_path):
                convert_cmd = [sys.executable, convert_script, trainer.log_path]
                print(f"[INFO] Running convertlog2eval: {' '.join(convert_cmd)}")
                subprocess.run(convert_cmd, check=True)
            else:
                print("[WARN] Skip convertlog2eval because script/log file is missing.")
