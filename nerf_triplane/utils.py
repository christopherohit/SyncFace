...
class Trainer(object):
    ...
    def _metric_name(self, metric):
        ...
        if isinstance(metric, LPIPSMeter):
            return f"LPIPS_{metric.net}"
        if isinstance(metric, LMDMeter):
            return f"LMD_{metric.backend}"
        return metric.__class__.__name__

    def _append_eval_csv(self, loader, average_loss):
        if self.workspace is None or self.local_rank != 0:
            return

        eval_csv_path = os.path.join(self.workspace, "eval_metrics.csv")
        metric_fields = [self._metric_name(metric) for metric in self.metrics]
        field_names = ["epoch", "split", "checkpoint", "val_loss"] + metric_fields

        split = "eval"
        if hasattr(loader, "_data") and hasattr(loader._data, "type"):
            split = str(loader._data.type)

        row = {
            "epoch": self.epoch,
            "split": split,
            "checkpoint": f"{self.ckpt_path}/{self.name}_ep{self.epoch:04d}.pth",
            "val_loss": f"{average_loss:.8f}",
        }

        for metric in self.metrics:
            metric_name = self._metric_name(metric)
            metric_value = ""
            if hasattr(metric, "N") and metric.N > 0:
                metric_value = f"{metric.measure():.8f}"
            row[metric_name] = metric_value

        is_new_file = (not os.path.exists(eval_csv_path)) or os.path.getsize(eval_csv_path) == 0
        with open(eval_csv_path, "a", newline="") as csv_file:
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            if self.workspace is not None and self.local_rank == 0:
                # keep every epoch checkpoint for later best-weight selection.
            ...
            if self.workspace is not None and self.local_rank == 0:
                ...
                review_name = f'{self.name}_ep{self.epoch:04d}_review'
                self.test(valid_loader, save_path=review_dir, name=review_name)
    ...
    def evaluate_one_epoch(self, loader, name=None):
        ...
        if self.local_rank == 0:
            ...
            self._append_eval_csv(loader, average_loss)
